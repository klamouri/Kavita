...
| `{hardcoverId-123}` | `Series.HardcoverId` (series id only) |
...
`{hardcoverId-…}` is a Hardcover **series** id and only sets
`Series.HardcoverId`. Since Kavita v0.9.1, `Chapter.HardcoverId` holds a
Hardcover **book** id (read from ComicInfo `Web` links and used for
scrobbling), so the token is never copied onto chapters. Older fork builds did
copy it; the next strict scan clears chapter Hardcover ids that equal the token
unless the same id was read from that chapter's ComicInfo. Applying the token
also clears `Series.IsStandAlone`.
